/**
* @pollyjs/adapter-node-http v2.7.0
*
...
function normalizeClientRequestArgs(input, options, cb) {
  if (typeof input === 'string') {
    input = urlToOptions(new url__default.URL(input));
  } else if (input instanceof url__default.URL) {
...
var normalizeClientRequestArgs_1 = normalizeClientRequestArgs;
var isUtf8Representable_1 = isUtf8Representable;
...
function getUrlFromOptions(options = {}) {
  ...
  const url = new utils.URL();
  url.set('protocol', protocol);
  url.set('host', host);
  url.set('pathname', path);

  if (port && !host.includes(':') && (port !== 80 || protocol !== 'http:') && (port !== 443 || protocol !== 'https:')) {
    url.set('port', port);
  }
...
const REQUEST_ARGUMENTS = new weakMap$1(); // nock begins to intercept network requests on import which is not the
...
class HttpAdapter extends Adapter {
  ...
  setupNock() {
    const adapter = this; // Make sure there aren't any other interceptors defined

    nock.cleanAll(); // Create our interceptor that will match all hosts

    const interceptor = nock(/.*/).persist();
    utils.HTTP_METHODS.forEach(m => {
      ...
      interceptor.intercept(/.*/, m).reply(function (_, body, respond) {
        ...
        const parsedArguments = normalizeClientRequestArgs_1(...REQUEST_ARGUMENTS.get(req));
        const url = getUrlFromOptions(parsedArguments.options);

        if (body) {
          if (typeof body === 'string' && !isUtf8Representable_1(Buffer.from(body, 'hex'))) {
          ...
          } else if (isJSONContent_1(headers)) {
  ...
  patchOverriddenMethods() {
    ...
    const {
      ClientRequest
    } = http; // Patch the already overridden ClientRequest class so we can get
    ...
    http.ClientRequest[IS_STUBBED] = true; // Patch http.request, http.get, https.request, and https.get
    // to set some default values which nock doesn't properly set.

    keys$1(modules).forEach(moduleName => {
      ...
      function parseArgs() {
        const args = normalizeClientRequestArgs_1(...arguments);

        if (moduleName === 'https') {
          args.options = objectSpread({}, {
            port: 443,
            protocol: 'https:',
  ...
  async passthroughRequest(pollyRequest) {
    ...
    const request = new this.NativeClientRequest(objectSpread({}, options, {
      method,
      headers: objectSpread({}, headers)
    }, urlToOptions$1(new url.URL(pollyRequest.url))));
    ...
    const responseBody = await new promise$1((resolve, reject) => {
      const chunks = [];
      response.on('data', chunk => chunks.push(chunk));
      response.once('end', () => resolve(this.getBodyFromChunks(chunks, response.headers)));
  ...
  async respondToRequest(pollyRequest, error) {
    ...
    stream$1.push(null); // Create a promise that will resolve once the request
    ...
    const requestFinishedPromise = new promise$1(resolve => {
      if (req.aborted) {
        resolve();
      } else {
    ...
    respond(null, [statusCode, stream$1, headers]);
  ...
  getBodyFromChunks(chunks, headers) {
    ...
    return buffer.toString(isUtf8Representable_1(buffer) ? 'utf8' : 'hex');
  }
  ...
  getChunksFromBody(body, headers) {
    ...
    return [Buffer.from(buffer, isUtf8Representable_1(buffer) ? 'utf8' : 'hex')];
  }

}

module.exports = HttpAdapter;
//# sourceMappingURL=pollyjs-adapter-node-http.js.map
